    ...
    try {
        ...
        const articulos = await Articulo.findAll({
            ...
            include: [{ 
                model: Proveedor, 
                as: 'proveedor', 
                attributes: ['nombre_proveedor'] // <--- ASEGURATE QUE DIGA 'proveedor' y no 'provider'
            }]
        });

        // ... resto de la lógica de mapeo para la rentabilidad ...
...
exports.createArticulo = async (req, res) => {
    try {
        ...
        res.status(400).json({ error: "Error al crear artículo. Verifique que el SKU sea único." });
    }
};
